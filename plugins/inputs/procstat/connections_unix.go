...
// Needs especial capabilities to work with all procs: sudo setcap "CAP_DAC_READ_SEARCH,CAP_SYS_PTRACE+ep" telegraf
...
func mapInodesToPid() (ret map[uint32][]InodeInfo) {
	...
	for _, pidStr := range dirContents {
		...
		// Get parent pid (best effort)
		var ppid uint64
		stat, _ := ioutil.ReadFile("/proc/" + pidStr + "/stat")
		statSplit := strings.Split(string(stat), ")")
		// Proc name could have white spaces and ')' symbols
		if len(statSplit) > 1 {
			values := strings.Split(statSplit[len(statSplit)-1], " ")
			if len(values) > 2 {
				ppid, _ = strconv.ParseUint(values[2], 10, 32)
			}
		}
